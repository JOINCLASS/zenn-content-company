...
合同会社ジョインクラスは、「個の開発を促進し、社会の発展に貢献する」をミッションに掲げるテックカンパニーです。CEO1名とAIエージェント群で6つのプロダクトと受託開発を運営しています。

本書で解説した技術スタック（Next.js + Supabase + Stripe）は、当社の営業支援SaaS「Focalize」で実際に採用しているものです。

### SaaS開発・DX支援のご相談

「自社でSaaSプロダクトを開発したいが、技術選定や設計で迷っている」「既存の業務システムをモダン化したい」――そんなご相談に対して、本書の内容を実践で活用している著者が直接サポートします。

**提供サービス:**

| サービス | 内容 | 対象 |
|---------|------|------|
| **技術相談（無料・60分）** | 技術選定、アーキテクチャ設計、開発フローの相談 | SaaS開発を検討中の企業・個人 |
| **DX支援・受託開発** | 要件定義から開発・運用まで伴走支援 | 業務システムのモダン化を進めたい中小企業 |
| **AI業務自動化コンサルティング** | 業務フローのAI化支援 | AI導入を検討中の企業 |
